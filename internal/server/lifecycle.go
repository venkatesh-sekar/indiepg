...
func Update(ctx context.Context, opts UpdateOptions) error {
	...
	// it bypasses the captured exec.Runner by design.
	log.Info("downloading indiepg release binary", "version", version)
	dl := osexec.CommandContext(ctx, "sh", "-c", script)
	dl.Stdin, dl.Stdout, dl.Stderr = os.Stdin, os.Stdout, os.Stderr
	dl.Env = append(os.Environ(), "INDIEPG_NO_INSTALL=1")
	if opts.Version != "" {
		dl.Env = append(dl.Env, "INDIEPG_VERSION="+opts.Version)
	}
	...
	log.Info("indiepg updated and service restarted", "service", systemdServiceName)
	announceUpdateSummary(true)
	return nil
}
...
// operator that only the binary changed.
func announceUpdateSummary(serviceRestarted bool) {
	const banner = "============================================================"
	out := os.Stdout
	out.WriteString("\n" + banner + "\n")
	out.WriteString("  indiepg updated to the latest release binary.\n")
	if serviceRestarted {
	...
	out.WriteString("  Confirm the version:   indiepg version\n")
