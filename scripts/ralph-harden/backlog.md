...
- [x] (1 · A) pgbouncer — enable's "verify running before recording success" invariant. Iter #4 covered the *first-run* half (`Reload`'s post-apply verify) via `TestEnable_ServiceNotRunningAfterStartIsNotRecorded`. Iter #14 closed the residual: the **final `IsRunning` gate** (enable.go:208-216), the ONLY dead-pooler catch on the idempotent no-config-change re-run (Reload skipped when nothing changed) — deleting it left the whole suite green. `enable_test.go`: `TestEnable_NoConfigChangeReRunOverDeadPoolerFailsAtFinalGate` (definitive-dead → `!running` arm, `CodeInternal`/"did not come up after enable", no re-persist) + `TestEnable_NoConfigChangeReRunUndeterminableStateFailsAtFinalGate` (empty-stdout+err → error arm, `CodeExec`/"could not determine service state"). Mutation-proven; test-skeptic's `running, _ :=` swallow-error escape closed by the error-arm companion before commit. No bug. Iter #14.
...
- [ ] (2 · A) install/upgrade — `MajorUpgradePreflight` (pg/preflight.go:212-363): its runner-driven psql substrate is now covered by `internal/pg/preflight_test.go` (Iter #13) — `scalarCount` (backs the prepared-xacts + logical-slot blockers; **fails loud** on unparseable/empty output instead of a silent `(0,nil)` that would defeat both blockers) and `installedExtensions`→`listDatabaseNames` (backs extension parity + preview; dedups/sorts across DBs, **fails loud** on a per-DB or db-enumeration error rather than under-reporting parity; the `<> 'plpgsql'` exclusion is pinned on the recorded argv, closing the test-skeptic's SQL-decoupled escape). Mutation-proven over 8 mutations. RESIDUAL (keep open, lower value): the FS-reading checks — `fileExists(pgUpgradeBin)` (target binaries), `freeBytes`/`DataDirectory`/`du` (disk headroom), and the on-disk `<ext>.control` parity scan — read hardcoded absolute paths with no injectable seam, so a full end-to-end `MajorUpgradePreflight`/`CheckSet.HasFail()` test would be host-flaky until FS seams are added on `Manager`/preflight (same residual as InstallPreflight, Iter #12).
- [x] (2 · A) alert — `handleTestAlertChannel` (handlers_alerts.go:291-342): confirmed the alleged nil-deref is **unreachable** and pinned the two guards that keep it so, via `internal/server/handlers_alerts_test.go` (3 tests driving the REAL router `POST /api/alerts/channels/test`). (1) `..._RejectsUnknownKind` — an unknown `kind` is rejected 400/`CodeValidation` (naming both accepted kinds) before any channel load, so `req.Kind ∈ {pushover,webhook}`. (2) `..._RequiresChannelOfExactRequestedKind` — with only a webhook configured, a pushover test returns 404/`CodeNotFound` and never dispatches (exact-kind selection; the ONLY NotFound path is `ch==nil`, so any mis-selection reds as 500 ExecError / 400, never 404). (3) `..._DispatchesToMatchingChannel` — a matching webhook IS selected and its `SendTest` fires exactly once (httptest hit-count + `event:"test"` payload). Mutation-proven over 4 correctly-aimed mutations (test-handler copies, lines 291-342; the validation+selection lines are DUPLICATED in the save handler — the Iter #13 non-`/g` mis-aim trap bit here, caught and fixed). No bug. code-reviewer + test-skeptic both clean (skeptic found no escaping mutation). Iter #15.
