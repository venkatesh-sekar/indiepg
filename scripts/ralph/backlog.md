...
- [ ] (1) Close the read-only CREATE-via-PUBLIC residual: on PG <= 14 the `public` schema grants CREATE to `PUBLIC`, so `indiepg_readonly` can still CREATE (and thus own + write) scratch tables in `public` once it resets its own `default_transaction_read_only` GUC. `provisionSQL` only does `REVOKE CREATE ON SCHEMA public FROM indiepg_readonly`, which does NOT remove the privilege inherited via `PUBLIC` — the provisionSQL comment claiming it closes this is wrong on PG14. Fix must disarm the PUBLIC-derived CREATE for the read-only role without breaking the admin role's / operator apps' object creation (a plain `REVOKE ... FROM PUBLIC` also disarms `indiepg_admin`, so it needs an explicit re-GRANT to admin, and care since the read pools also open per-database). Scope to the panel-managed `postgres` DB plus any browsed app DB the read pool connects to. — acceptance: extend `TestReadOnlyRole_DBLevelWriteDenial` to assert `CREATE TABLE` by the read-only role is also denied (42501), green against a real cluster; admin object creation still works.
